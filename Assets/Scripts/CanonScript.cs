...
public class CanonScript : MonoBehaviour {
	...
	public AudioClip tiroCanon;
	public GameObject eggCanon;
	...
	public void Atira(Vector2 mouseVectorPosition, Vector3 angulo){
		...
		eggCanon.SetActive (true);
		eggInstance = Instantiate (eggCanon);
		eggInstance.transform.eulerAngles = angulo;
		eggCanon.SetActive (false);
		
		rbEgg = eggInstance.GetComponent<Rigidbody2D> ();
		rbEgg.AddForce (new Vector2(mouseVectorPosition.x,mouseVectorPosition.y + 2.5f).normalized * 25000);
		
		AudioSource.PlayClipAtPoint (tiroCanon, transform.position, 1.0f);
